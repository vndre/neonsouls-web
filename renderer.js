...
export function renderer(ctx, width, height) {
  ...
  const gridWidth = Math.max(
    minGridWidth,
    Math.min((width / 16) | 0, maxGridWidth)
  );
  const maxGridHeight = 64;
  const minGridHeight = 32;
  const gridHeight = Math.max(
    minGridHeight,
    Math.min((height / 16) | 0, maxGridHeight)
  );
  ...
  function updateTerrain(t) {
    ...
    iterateGrid2D(grid, (p, x, y, i) => {
      ...
      const roadTwistyness =
        6 * Math.max(0, Math.sin((y - terrainOffset) / twistynessPeriod));
      const cornerPeriod = 10;
      const roadWinding =
        Math.sin((y - terrainOffset) / cornerPeriod) * roadTwistyness;
      const road =
        Math.max(
          roadWidth - 1,
      ...
      const hilliness = Math.abs(
        Math.sin((y - terrainOffset) / hillinessPeriod)
      );
      const mountainHeight = 5;
      const mountainOffset = 2;
      const mountains =
        mountainOffset +
        noise2D(x * noiseScale, (y - terrainOffset) * noiseScale) *
          mountainHeight *
          hilliness;
  ...
  function drawGrid2D() {
    ...
    ctx.strokeStyle = '#871fff';
    ctx.fillStyle = '#ffa3e0';

    for (let y = 0; y < gridHeight - 1; y++) {
      const z = points[index(0, y) + 2];
      const fadeDistance = 0.8;
      ctx.globalAlpha =
        1 - Math.max(0, z - fadeDistance) * (1 / (1 - fadeDistance));
